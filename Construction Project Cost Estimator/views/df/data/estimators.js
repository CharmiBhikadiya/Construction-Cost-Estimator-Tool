const mongoCollections = require("../mongo-config/mongoCollections");
...
const exportedMethods = {
    ...
    async getEstimatorVerification(email) {
        ...
        if (!estimator) throw "No Vendor found with given email";
    ...
    async updateEstimator(id, estimatorDataToUpdate) {
        ...
        if (estimatorDataToUpdate.hasOwnProperty("firstName")) {
            someToUpdate = true;
            if (typeof estimatorDataToUpdate.title != "string" || estimatorDataToUpdate.title.length <= 0) {
        ...
        if (estimatorDataToUpdate.hasOwnProperty("userName")) {
            someToUpdate = true;
            if (typeof estimatorDataToUpdate.userName != "userName" || estimatorDataToUpdate.title.length <= 0) {
                console.log("Invalid user name");
                throw "Invalid user name";
            }
            estimatorReadyToUpdate.userName = estimatorDataToUpdate.userName;
        }
